...
function AtbashExample() {
  const context = useContext(AlphabetContext);
  if(!context) {
    throw new Error("Missing Alphabet context");
  }

  const cipherText = affineCipher(context.alphabet.join(''), context.alphabet, atbash);

  return (
    <CipherTable
      plaintext={context.alphabet}
...
function TryAtbash() {
  ...
  function encode(text: string) {
    return affineCipher(text, context?.alphabet || [], atbash);
  }

  return (
    <TryItOutProvider encode={encode} decode={encode}>
      <TryItOut alphabet={context.alphabet}></TryItOut>
    </TryItOutProvider>
  );
}
...
export default function Component() {
  return (
    <FootnoteProvider>
      <AlphabetProvider defaultAlphabet={alphabets.latin}>
